...
tr.reorder();
tr.send<2>((char*)__xlx_apatb_param_out_ar, 8192);
}
#else
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<16> > out_ar_pc_buffer(8192);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 8192; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_out_ar)[j*2+0] = out_ar_pc_buffer[i].range(7, 0).to_int64();
...
unsigned __xlx_offset_byte_param_inA_ar = 0;
#ifdef USE_BINARY_TV_FILE
{
aesl_fh.touch(AUTOTB_TVIN_inA_ar, 'b');
transaction<16> tr(8192);
  __xlx_offset_byte_param_inA_ar = 0*2;
  if (__xlx_apatb_param_inA_ar) {
tr.import<2>((char*)__xlx_apatb_param_inA_ar, 8192, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_inA_ar, tr.p, tr.tbytes);
}

  tcl_file.set_num(8192, &tcl_file.inA_ar_depth);
#else
...
tr.import<2>((char*)__xlx_apatb_param_out_ar, 8192, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_out_ar, tr.p, tr.tbytes);
}

  tcl_file.set_num(8192, &tcl_file.out_ar_depth);
#else
