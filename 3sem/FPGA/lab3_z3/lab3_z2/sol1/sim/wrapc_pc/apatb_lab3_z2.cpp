    ...
    string AESL_num;
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<16> > inA_ar_pc_buffer(4096);
          int i = 0;
          ...
          if (i > 0) {{
		    int i = 0;
            for (int j = 0, e = 4096; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_inA_ar)[j*2+0] = inA_ar_pc_buffer[i].range(7, 0).to_int64();
...
tr.reorder();
tr.send<2>((char*)__xlx_apatb_param_out_ar, 4096);
}
#else
{
      static ifstream rtl_tv_out_file;
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<16> > out_ar_pc_buffer(4096);
          int i = 0;
          bool has_unknown_value = false;
          rtl_tv_out_file >> AESL_token; //data
          while (AESL_token != "[[/transaction]]"){
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 4096; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_out_ar)[j*2+0] = out_ar_pc_buffer[i].range(7, 0).to_int64();
...
aesl_fh.write(AUTOTB_TVIN_inA_ar, __xlx_tmp_lv.to_string(SC_HEX)+string("\n"));
  ...
  tcl_file.set_num(4096, &tcl_file.inA_ar_depth);
...
tr.import<2>((char*)__xlx_apatb_param_out_ar, 4096, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_out_ar, tr.p, tr.tbytes);
}

  tcl_file.set_num(4096, &tcl_file.out_ar_depth);
#else
