...
// ==============================================================
#ifndef foo_m_fifo_w32_d16384_A_HH_
#define foo_m_fifo_w32_d16384_A_HH_

#include <systemc>
using namespace std;
SC_MODULE(foo_m_fifo_w32_d16384_A) {
    static const unsigned int DATA_WIDTH = 32;
    static const unsigned int ADDR_WIDTH = 15;
    static const unsigned int foo_m_fifo_w32_d16384_A_depth = 16385;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[foo_m_fifo_w32_d16384_A_depth];
    ...
    SC_CTOR(foo_m_fifo_w32_d16384_A) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < foo_m_fifo_w32_d16384_A_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (foo_m_fifo_w32_d16384_A_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < foo_m_fifo_w32_d16384_A_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (foo_m_fifo_w32_d16384_A_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > foo_m_fifo_w32_d16384_A_depth) {
...
#endif //foo_m_fifo_w32_d16384_A_HH_
