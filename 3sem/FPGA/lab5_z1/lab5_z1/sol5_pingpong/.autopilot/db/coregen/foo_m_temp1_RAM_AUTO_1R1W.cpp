...
#include "foo_m_temp1_RAM_AUTO_1R1W.h"

using namespace std;
using namespace sc_core;
using namespace sc_dt;

foo_m_temp1_RAM_AUTO_1R1W::~foo_m_temp1_RAM_AUTO_1R1W() {
    if (m_trace_file) sc_close_vcd_trace_file(m_trace_file);
}

void foo_m_temp1_RAM_AUTO_1R1W::proc_i_full_n() {
    i_full_n.write(full_n.read());
}

void foo_m_temp1_RAM_AUTO_1R1W::proc_t_empty_n() {
    t_empty_n.write(empty_n.read());
}

void foo_m_temp1_RAM_AUTO_1R1W::proc_memcore_addr() {
    memcore_iaddr = (i_address0.read(), iptr.read());
    memcore_taddr = (t_address0.read(), tptr.read());
}

void foo_m_temp1_RAM_AUTO_1R1W::proc_push_buf() {
    push_buf.write(i_ce.read() & i_write.read() & full_n.read());
}

void foo_m_temp1_RAM_AUTO_1R1W::proc_pop_buf() {
    pop_buf.write(t_ce.read() & t_read.read() & empty_n.read());
}

void foo_m_temp1_RAM_AUTO_1R1W::proc_iptr() {
...
void foo_m_temp1_RAM_AUTO_1R1W::proc_tptr() {
    if (reset.read() == SC_LOGIC_1) {
        tptr.write(0);
...
void foo_m_temp1_RAM_AUTO_1R1W::proc_count() {
    if (reset.read() == SC_LOGIC_1) {
        count.write(0);
...
void foo_m_temp1_RAM_AUTO_1R1W::proc_full_n() {
    if (reset.read() == SC_LOGIC_1) {
...
void foo_m_temp1_RAM_AUTO_1R1W::proc_empty_n() {
    if (reset.read() == SC_LOGIC_1) {
